...
for (const people of data.people) {
  const count = people.hobbies.length;
  console.log(count);
}
// 3. Create a mutable copy of Visard and change his name to "Visard of Oz"
const changeVisard = JSON.parse(JSON.stringify(Visard));
changeVisard.name = "Visard of Oz";
// 4. Capitalise all hobbies in the data object
for (const people of data.people) {
  console.log(people.hobbies.toString().toUpperCase());
}
...
// }
//
const peopleByName = Object.freeze({
  Ed: {
    name: "Ed",
    id: 6,
    hobbies: ["Drawing birds", "Shitposts", "Muay thai"],
  },
  Nico: {
    name: "Nico",
    id: 7,
    hobbies: ["Coding", "Learning Japanese", "Being pretty lit"],
  },
  Marsel: {
    name: "Marsel",
    id: 3,
